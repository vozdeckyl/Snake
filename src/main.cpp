...
int mainMenu(IGraphicsEngine& engine)
{
    ...
    preGameWindow.addElement(new Label(copyright), preGameWindow.getHeight() - 1,
                             preGameWindow.getWidth() - copyright.length() - 1);
...
void records(IGraphicsEngine& engine)
{
    ...
    for (pair<string, string> scoreRecord : scoresList)
    {
        ...
        records.addElement(new Label(scoreRecord.second, Color::magenta), verticalPosition + offset,
